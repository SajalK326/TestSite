...
(function () {
  ...
  function initBackground(canvas) {
    ...
    var gridMat = new THREE.ShaderMaterial({
      ...
      uniforms: {
        uTime: { value: 0 },
        uColorFar: { value: new THREE.Color(0x00f0ff) },
        uColorNear: { value: new THREE.Color(0xff006e) },
      },
      vertexShader:
    ...
    // Horizon glow
    var horizonGeom = new THREE.PlaneGeometry(200, 0.3);
    var horizonMat = new THREE.MeshBasicMaterial({
      color: 0xff006e, transparent: true, opacity: 0.6, blending: THREE.AdditiveBlending,
    });
  ...
  function boot() {
    ...
    if (hasWebGL()) {
      ...
      if (hero) {
        hero.style.background = 'radial-gradient(circle at 70% 50%, rgba(131,56,236,0.3), transparent 50%), radial-gradient(circle at 30% 30%, rgba(0,240,255,0.2), transparent 60%), #050510';
      }
    }
  }
